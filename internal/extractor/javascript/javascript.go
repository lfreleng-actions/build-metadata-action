...
func readTSConfig(path string) (map[string]interface{}, error) {
	...
	contentStr = removeJSONComments(contentStr)
...
// removeJSONComments removes // and /* */ style comments from JSON
func removeJSONComments(jsonStr string) string {
	...
	for _, line := range lines {
		...
		// Preserve empty lines, and lines that had content even if now empty after comment removal
		if len(strings.TrimSpace(processedLine)) > 0 || !hadContent {
			result = append(result, processedLine)
		} else if hadContent {
			// Line had content but is now empty due to comment removal - preserve as empty
...
// findLineCommentOutsideString finds the index of "//" that's not inside a JSON string
// Returns -1 if no line comment found outside of strings
