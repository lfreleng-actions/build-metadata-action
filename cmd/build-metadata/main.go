...
func main() {
	...
	for key, value := range metadata.LanguageSpecific {
		...
		switch v := value.(type) {
		case string:
			setOutput(fmt.Sprintf("%s_%s", prefix, key), v)
		case []string:
			setOutput(fmt.Sprintf("%s_%s", prefix, key), strings.Join(v, ","))
		case map[string]interface{}:
			// Convert complex types to JSON
			jsonBytes, _ := json.Marshal(v)
			setOutput(fmt.Sprintf("%s_%s", prefix, key), string(jsonBytes))
		default:
			setOutput(fmt.Sprintf("%s_%s", prefix, key), fmt.Sprintf("%v", v))
		}
	}
